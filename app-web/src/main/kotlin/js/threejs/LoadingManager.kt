...
open external class LoadingManager(
    ...
    onProgress: ((url: String, loaded: Number, total: Number) -> Unit)? = definedExternally,
    onError: ((url: String) -> Unit)? = definedExternally
) {
    open var onStart: (url: String, loaded: Number, total: Number) -> Unit
    open var onLoad: () -> Unit
    open var onProgress: (item: Any, loaded: Number, total: Number) -> Unit
